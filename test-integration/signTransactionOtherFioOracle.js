...
const abiOracleMainnet = await (await fetch(httpEndpointMainnet + '/v1/chain/get_abi', {
    body: `{"account_name": "fio.oracle"}`,
    method: 'POST',
})).json()
const abiOracleTestnet = await (await fetch(httpEndpointTestnet + '/v1/chain/get_abi', {
    body: `{"account_name": "fio.oracle"}`,
    method: 'POST',
})).json()
...
const typesTransaction2Mainnet = ser.getTypesFromAbi(ser.createInitialTypes(), abiOracleMainnet.abi)
const typesTransaction2Testnet = ser.getTypesFromAbi(ser.createInitialTypes(), abiOracleTestnet.abi)

console.log(typesTransaction2Mainnet.get('wrapdomain'));
console.log(typesTransaction2Mainnet.get('wraptokens'));
...
testStep(" - - -", "Sign testnet transaction - wrapdomain");
{
    ...
    const tx = {
        ...
        actions: [{
            ...
            data: {
                fio_domain: "Test domain",
                chain_code: "Chain",
                public_address: "My address",
                max_oracle_fee: 0x44223344,
                max_fee: 0x11223344,
                tpid: "rewards@wallet",
                actor: "aftyershcu22",
            },
    ...
    await device.review([1, 1, 1, 1, 1, 1, 1, 2], "Review sign");
...
testStep(" - - -", "Sign mainnet transaction - wraptokens");
{
