...
import { Fio, DeviceStatusError, InvalidData } from "../ledgerjs-fio/dist/src/fio.js"
...
const getPubkeyPromise = app.getPublicKey({path: str_to_path(path), show_or_not: false});
...
const getPubkeyPromise2 = app.getPublicKey({path: str_to_path(path), show_or_not: true});
...
const getPubkeyPromise3 = app.getPublicKey({path: str_to_path(path2), show_or_not: false});
...
const getPubkeyPromise4 = app.getPublicKey({path: str_to_path(path2), show_or_not: true});
await device.review([1, 1, 2,], "Review pubkey");
...
const promise1 = app.getPublicKey({path: str_to_path("44'/235'/0'/0"), show_or_not: false});
...
const promise2 = app.getPublicKey({path: str_to_path("44'/235'/1'/0/0"), show_or_not: false})
...
const promise3 = app.getPublicKey({path: str_to_path("44'/235'/0/0/0"), show_or_not: false})
...
const promise4 = app.getPublicKey({path: str_to_path("44'/235'/0'/1/0"), show_or_not: false})
await assert.rejects(promise4, DeviceStatusError, "Action rejected by Ledger's security policy");

testStep(" - - -", "path too long");
const promise5 = app.getPublicKey({path: str_to_path("44'/235'/0'/0/0/0"), show_or_not: false})
