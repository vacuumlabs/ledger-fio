...
//It was not very convenient to use higher level funtions for this purpose as they depend on fio serialization
...
static void run_dh_encode_tests() {
    BEGIN_ASSERT_NOEXCEPT {
        ...
        const char* publicKeyHex = "0484e52dfea57b8f1787488a356374cd8e8515b8ad8db3dd4f9088d8e42ed2fb6d571e8894cccbdbf15e1bd84f8b4362f52d1b5b712b9775c0a51cdd5ee9a9e8ca";
        ...
        #define TESTCASE(msgHex_, expectedEncryptedMsgHex_)                                 \
            {                                                                               \
                ...
                uint8_t encMsg[300];                                                        \
                size_t encMsgLength = dh_encode(&pathSpec, &publicKey, IV, DH_AES_IV_SIZE,  \
                                                msg, msgLen, encMsg, SIZEOF(encMsg));       \
        ...
        TESTCASE("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef0011",
                 "000102030405060708090a0b0c0d0e0f9508b492f96f067cc72ef8c7c24ac2072310c4e1d36bd6737958f0a3a005576d2b88c5f509bb7276a32f94595e196cf382b0625992dd194210a0e545410741a8013a122d2edd9233caa3fd4b75f9946c")
...
void run_dh_encode_init_append_finalize_tests() {
    BEGIN_ASSERT_NOEXCEPT {
        ...
        const uint8_t IV[DH_AES_IV_SIZE] = {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100};

        dh_aes_key_t key;
        dh_init_aes_key(&key, &pathSpec, &publicKey);
        dh_context_t ctx;

        const char *inBufferHex = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef00112233445566778899aabbccddeeff";
        uint8_t inBuffer[200];                                                           \
        size_t inBufferLength = decode_hex(inBufferHex, inBuffer, SIZEOF(inBuffer));
        const char *expectedHex = "6464646464646464646464646464646440a725eec9623619fd422c0ce7bf965506c15165fa5277429c66e6ed1f2c6f64c6e10668dd7c5e704c12eea22579a11118da4211e13782ff8193ecf400fd04dbcf317a5df47f449d1dbb75e9f48428d0a840a0e90139759ad39d28990cb37f62";
        ...
        outBufferLength = dh_encode(&pathSpec, &publicKey, IV, DH_AES_IV_SIZE, inBuffer, inBufferLength, outBuffer, SIZEOF(outBuffer));
        ...
        outBufferLength = dh_encode_append(&ctx, &key, inBuffer+read, 1, outBuffer, SIZEOF(outBuffer));
        ASSERT(outBufferLength == 0);
        read += 1;

        outBufferLength = dh_encode_append(&ctx, &key, inBuffer+read, 14, outBuffer, SIZEOF(outBuffer));
        ASSERT(outBufferLength == 0);
        read += 14;

        outBufferLength = dh_encode_append(&ctx, &key, inBuffer+read, 18, outBuffer, SIZEOF(outBuffer));
