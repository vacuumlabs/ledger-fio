...
__noinline_due_to_stack__ void dh_init_aes_key(dh_aes_key_t *dhKey, const bip44_path_t *pathSpec, const public_key_t *publicKey) {
    ...
    BEGIN_TRY {
        TRY {
            ...
            cx_err_t err = cx_ecdh_no_throw(&privateKey, CX_ECDH_X, publicKey->W, publicKey->W_len, basicSecret, SIZEOF(basicSecret));
            ASSERT(err == CX_OK);
            sha_512_hash(basicSecret, SIZEOF(basicSecret), secret, SIZEOF(secret));
	        sha_512_hash(secret, SIZEOF(secret), K, SIZEOF(K));

            //First DH_AES_SECRET_SIZE bytes are used to compute shared secret, then DH_KM_SIZE are used as Km for HMAC calculation
...
size_t dh_encode_init(dh_context_t* ctx, const dh_aes_key_t* aes_key, const uint8_t* iv, size_t ivSize,
...
size_t dh_encode_append(dh_context_t* ctx, const dh_aes_key_t* aes_key,
                        const uint8_t* inBuffer, size_t inSize, uint8_t* outBuffer, size_t outSize) {
...
size_t dh_encode_finalize(dh_context_t* ctx, const dh_aes_key_t* aes_key, uint8_t* outBuffer, size_t outSize) {
...
size_t dh_encode(bip44_path_t *pathSpec, public_key_t *publicKey, const uint8_t* iv, size_t ivSize,
                 const uint8_t* inBuffer, size_t inSize, uint8_t* outBuffer, size_t outSize) {                                                                
    dh_aes_key_t key;
    TRACE("dh_encode");
    ...
    BEGIN_TRY {
        TRY {
            ...
            dh_context_t ctx;
            written += dh_encode_init(&ctx, &key, iv, ivSize, outBuffer+written, outSize-written);
            written += dh_encode_append(&ctx, &key, inBuffer, inSize, outBuffer+written, outSize-written);
