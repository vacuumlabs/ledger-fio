...
static void getPublicKey_ui_runStep() {
    ...
    UI_STEP(GET_KEY_UI_STEP_WARNING) {
        ui_displayPaginatedText("Unusual request", "Proceed with care", this_fn);
    }
    ...
    UI_STEP(GET_KEY_UI_STEP_RESPOND) {
        ...
        uint32_t wifkeylen = public_key_to_wif(ctx->pubKey.W,
                                               SIZEOF(ctx->pubKey.W),
...
void getPublicKey_handleAPDU(uint8_t p1,
                             uint8_t p2,
